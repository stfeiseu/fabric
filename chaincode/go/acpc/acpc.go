...
func NewAccessPolicyContract() AccessPolicyContract {
...
func (cc *ChainCode) parsePolicy(arg string) (m.Policy, error) {
	...
	policy := m.Policy{}
	err := json.Unmarshal(policyAsBytes, &policy)//将json字符串解码到相应数据结构。
	return policy, err
}

func (cc *ChainCode) VerifyPolicy(p m.Policy) bool {
	return false
}
